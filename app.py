...
def _clean_forecast_df(df: pd.DataFrame) -> pd.DataFrame:
    """Keep fixed years/rows, numeric values and empty financial defaults.

    This function does NOT calculate %/Value pairs. Pair calculations are made
    after the data_editor returns the user's edit, so entered values do not
    disappear during reruns.
    """
...
def _sync_forecast_calculations(edited: pd.DataFrame, previous: pd.DataFrame) -> pd.DataFrame:
    """Synchronise % and Value columns immediately after a user edit.

    Rules per row and cost category:
    - If the user changed the % column and presses Enter, calculate Value.
    - If the user changed the Value column and presses Enter, calculate %.
    - If Sales changed, preserve the user's last input logic where possible:
      percentages drive values when the value was previously formula-derived;
      otherwise values drive percentages.
    """
    result = _clean_forecast_df(edited)
    previous = _clean_forecast_df(previous)

    pairs = [
        ("Material %", "Material Value"),
        ("Direct Labour %", "Direct Labour Value"),
        ("MOH %", "MOH Value"),
        ("SG&A %", "SG&A Value"),
    ]
    ...
    for idx in result.index:
        sales = float(result.at[idx, "Sales"] or 0.0)
        prev_sales = float(previous.at[idx, "Sales"] or 0.0) if idx in previous.index else 0.0
        sales_changed = changed(sales, prev_sales)

        for pct_col, val_col in pairs:
            pct = float(result.at[idx, pct_col] or 0.0)
            val = float(result.at[idx, val_col] or 0.0)
            prev_pct = float(previous.at[idx, pct_col] or 0.0) if idx in previous.index else 0.0
            prev_val = float(previous.at[idx, val_col] or 0.0) if idx in previous.index else 0.0

            pct_changed = changed(pct, prev_pct)
            val_changed = changed(val, prev_val)

            if pct_changed and not val_changed:
                result.at[idx, val_col] = sales * pct
            elif val_changed and not pct_changed:
                result.at[idx, pct_col] = (val / sales) if sales else 0.0
            elif pct_changed and val_changed:
                # When both arrive as changed, prefer the % field because the
                # main reported issue is losing a freshly entered percentage.
                result.at[idx, val_col] = sales * pct
            elif sales_changed:
                was_value_formula = abs(prev_val - (prev_sales * prev_pct)) <= 1e-6
                if was_value_formula or prev_pct > 0:
                    result.at[idx, val_col] = sales * pct
                else:
                    result.at[idx, pct_col] = (val / sales) if sales else 0.0
...
with tabs[2]:
    ...
    st.markdown('<div class="brand-line"></div>', unsafe_allow_html=True)
    st.info("Forecast Input relates only to incremental recurring revenues and costs generated by the new investment. Existing business volumes should not be included in this section. MOH should exclude depreciation costs. Depreciation used in Profit Forecast, Cash Flow and Balance Sheet comes directly from the Forecast Input column: Depreciation of Investment.")
    st.caption("Enter yearly assumptions. For Material, Direct Labour, MOH and SG&A choose whether you want to enter an absolute value or a % of Sales. The app calculates the other field automatically.")

    st.caption("Type either a % or an absolute value and press Enter. The paired field recalculates immediately and the table layout remains unchanged.")

    st.session_state.forecast_df = _clean_forecast_df(st.session_state.forecast_df)
    ...
    col_cfg = {
        ...
        "MOH %": st.column_config.NumberColumn("MOH %", format="%.4f", min_value=0.0, max_value=1.5, help="Manufacturing overhead % of Sales, excluding depreciation costs."),
        "MOH Value": st.column_config.NumberColumn("MOH Value", format="%.0f", min_value=0.0, help="Manufacturing overhead amount, excluding depreciation costs."),
        ...
        "Depreciation of Investment": st.column_config.NumberColumn("Depreciation of Investment", format="%.0f", min_value=0.0, help="Yearly depreciation entered by the user. This value feeds Profit Forecast, Cash Flow and Balance Sheet."),
    }
    previous_forecast_df = st.session_state.forecast_df.copy()
    ...
    edited = _sync_forecast_calculations(edited, previous_forecast_df)

    if not edited.equals(previous_forecast_df):
        st.session_state.forecast_df = edited
        st.rerun()

    issues_panel(validate_forecast(st.session_state.forecast_df))
