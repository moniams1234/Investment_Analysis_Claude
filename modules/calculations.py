...
def build_pnl(forecast_df: pd.DataFrame, setup: dict) -> pd.DataFrame:
    ...
    for _, r in forecast_df.iterrows():
        ...
        # 1) automatic project depreciation from Project Setup, unless the user enters
        #    Depreciation of Investment for the specific year in Forecast Input;
        # 2) plus depreciation/amortisation of other fixed assets entered by the user.
        # MOH is treated as excluding depreciation costs.
        idx_in_life = (year - int(forecast_df.iloc[0]["Year"])) < life
        auto_investment_dep = (depreciable / life) if idx_in_life and depreciable > 0 else 0.0
        manual_investment_dep = float(r.get("Depreciation of Investment", r.get("Investment Depreciation", 0)) or 0)
        other_fa_dep = float(r.get("Depreciation of Other Assets", r.get("Other FA Depreciation", 0)) or 0)
        investment_dep = manual_investment_dep if manual_investment_dep > 0 else auto_investment_dep
        depreciation = investment_dep + other_fa_dep
        ...
        rows.append({
            ...
            "SG&A": sga,
            "EBITDA": ebitda,
            "Depreciation of Investment": investment_dep,
            "Depreciation of Other Assets": other_fa_dep,
