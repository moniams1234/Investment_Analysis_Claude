...
LOGO_PATH = ROOT / "assets" / "fa_logo.png"
...
def _init_state() -> None:
    ...
    ss.setdefault("manual_material_value", True)
    ss.setdefault("manual_labour_value", True)
    ss.setdefault("manual_sga_value", True)
    ss.setdefault("manual_moh_value", True)
...
FORECAST_COLUMNS = [
    "Year", "Sales", "Material %", "Material Value",
    "Direct Labour %", "Direct Labour Value", "MOH %", "MOH Value",
    "SG&A %", "SG&A Value",
    "Investment Depreciation", "Other FA Depreciation",
]


def _clean_forecast_df(df: pd.DataFrame) -> pd.DataFrame:
    """Keep fixed years/rows and synchronize % vs value inputs.

    For each selected cost line:
    - checkbox ON  = user enters absolute Value, app calculates % of Sales
    - checkbox OFF = user enters %, app calculates absolute Value
    """
    clean = df.copy() if isinstance(df, pd.DataFrame) else pd.DataFrame()
    base = empty_forecast_df(st.session_state.num_years, st.session_state.start_year)

    for col in FORECAST_COLUMNS:
        if col not in clean.columns:
            clean[col] = 0.0

    clean = clean[FORECAST_COLUMNS].reset_index(drop=True)
    clean = clean.reindex(range(st.session_state.num_years))
    clean["Year"] = base["Year"].values

    for col in FORECAST_COLUMNS:
        if col != "Year":
            clean[col] = pd.to_numeric(clean[col], errors="coerce").fillna(0.0).clip(lower=0.0)

    sales = clean["Sales"].replace(0, pd.NA)

    def sync_pair(pct_col: str, value_col: str, manual_value_key: str) -> None:
        if st.session_state.get(manual_value_key, False):
            # User enters the absolute cost value; calculate % of Sales.
            clean[pct_col] = (clean[value_col] / sales).fillna(0.0)
        else:
            # User enters the cost %; calculate the absolute cost value.
            clean[value_col] = (clean["Sales"] * clean[pct_col]).fillna(0.0)

    sync_pair("Material %", "Material Value", "manual_material_value")
    sync_pair("Direct Labour %", "Direct Labour Value", "manual_labour_value")
    sync_pair("MOH %", "MOH Value", "manual_moh_value")
    sync_pair("SG&A %", "SG&A Value", "manual_sga_value")
...
with st.sidebar:
    ...
    st.caption("Upload a saved FIN APPS project file in JSON format.")
...
with tabs[2]:
    ...
    st.markdown('<div class="brand-line"></div>', unsafe_allow_html=True)
    st.info(
        "Forecast Input includes only recurring revenues and recurring operating costs related to the new investment. "
        "Do not enter one-off CAPEX here — CAPEX belongs in Project Setup."
    )
    st.caption(
        "All fields start empty. The app does not populate or replace Sales, Material %, Labour %, MOH % or SG&A % with default values. "
        "For Material, Direct Labour, MOH and SG&A choose whether you want to enter % of Sales or absolute Value. "
        "After pressing Enter in an input field, the paired value is recalculated immediately."
    )

    st.markdown("#### Cost input method")
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        material_mode = st.radio("Material", ["Enter %", "Enter Value"], horizontal=True,
                                 index=1 if st.session_state.manual_material_value else 0,
                                 key="material_input_mode")
        st.session_state.manual_material_value = material_mode == "Enter Value"
    with c2:
        labour_mode = st.radio("Direct Labour", ["Enter %", "Enter Value"], horizontal=True,
                               index=1 if st.session_state.manual_labour_value else 0,
                               key="labour_input_mode")
        st.session_state.manual_labour_value = labour_mode == "Enter Value"
    with c3:
        moh_mode = st.radio("MOH", ["Enter %", "Enter Value"], horizontal=True,
                            index=1 if st.session_state.manual_moh_value else 0,
                            key="moh_input_mode")
        st.session_state.manual_moh_value = moh_mode == "Enter Value"
    with c4:
        sga_mode = st.radio("SG&A", ["Enter %", "Enter Value"], horizontal=True,
                            index=1 if st.session_state.manual_sga_value else 0,
                            key="sga_input_mode")
        st.session_state.manual_sga_value = sga_mode == "Enter Value"

    # Keep the dataframe stable and make sure all required rows/columns exist.
    working_df = _clean_forecast_df(st.session_state.forecast_df)

    st.markdown("#### Annual recurring forecast")
    header = st.columns([0.75, 1.05, 1, 1, 1, 1, 1, 1])
    headers = [
        "Year", "Sales", "Material", "Direct Labour", "MOH", "SG&A",
        "Investment Dep.", "Other FA Dep."
    ]
    ...
    def _num(label: str, value: float, key: str, *, pct: bool = False, disabled: bool = False) -> float:
        return float(st.number_input(
            label,
            value=float(value or 0.0),
            min_value=0.0,
            max_value=1.5 if pct else None,
            step=0.01 if pct else 1000.0,
            format="%.4f" if pct else "%.0f",
            key=key,
    ...
    for i, row in working_df.iterrows():
        year = int(row["Year"])
        cols = st.columns([0.75, 1.05, 1, 1, 1, 1, 1, 1])
        cols[0].markdown(f"<div style='padding-top:0.45rem;font-weight:700'>{year}</div>", unsafe_allow_html=True)

        sales = _num("Sales", row.get("Sales", 0.0), f"fc_sales_{year}")

        def cost_pair(base_label: str, pct_col: str, val_col: str, manual_value: bool, col, key_prefix: str) -> tuple[float, float]:
            if manual_value:
                val = _num(f"{base_label} Value", row.get(val_col, 0.0), f"{key_prefix}_val_{year}")
                pct_val = (val / sales) if sales else 0.0
                col.caption(f"{pct_val:.2%} of Sales")
                return pct_val, val
            pct_val = _num(f"{base_label} %", row.get(pct_col, 0.0), f"{key_prefix}_pct_{year}", pct=True)
            val = sales * pct_val
            col.caption(f"{currency} {val:,.0f}")
            return pct_val, val

        mat_pct, mat_val = cost_pair("Material", "Material %", "Material Value", st.session_state.manual_material_value, cols[2], "mat")
        lab_pct, lab_val = cost_pair("Direct Labour", "Direct Labour %", "Direct Labour Value", st.session_state.manual_labour_value, cols[3], "lab")
        moh_pct, moh_val = cost_pair("MOH", "MOH %", "MOH Value", st.session_state.manual_moh_value, cols[4], "moh")
        sga_pct, sga_val = cost_pair("SG&A", "SG&A %", "SG&A Value", st.session_state.manual_sga_value, cols[5], "sga")

        investment_dep = _num("Investment Depreciation", row.get("Investment Depreciation", 0.0), f"fc_investment_dep_{year}")
        other_fa_dep = _num("Other FA Depreciation", row.get("Other FA Depreciation", 0.0), f"fc_other_fa_dep_{year}")
        ...
        updated_rows.append({
            ...
            "Direct Labour %": lab_pct,
            "Direct Labour Value": lab_val,
            "MOH %": moh_pct,
            "MOH Value": moh_val,
            "SG&A %": sga_pct,
            "SG&A Value": sga_val,
            "Investment Depreciation": investment_dep,
            "Other FA Depreciation": other_fa_dep,
        })

    edited = pd.DataFrame(updated_rows, columns=FORECAST_COLUMNS)
    st.session_state.forecast_df = edited

    with st.expander("Preview calculated Forecast Input table"):
        preview = edited.copy()
        for c in ["Sales", "Material Value", "Direct Labour Value", "MOH Value", "SG&A Value", "Investment Depreciation", "Other FA Depreciation"]:
            preview[c] = preview[c].map(lambda v: fmt_money(v, currency))
        for c in ["Material %", "Direct Labour %", "MOH %", "SG&A %"]:
            preview[c] = preview[c].map(fmt_pct)
...
with tabs[3]:
    ...
    if pnl.empty:
        ...
        for c in ["Sales","Material Cost","Gross Profit","Direct Labour","Manufacturing Overhead",
                  "SG&A","EBITDA","Investment Depreciation","Other FA Depreciation",
