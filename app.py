...
FORECAST_COLUMNS = [
    ...
    "SG&A %", "SG&A Value",
    "Depreciation of Investment", "Depreciation of Other Assets",
]
...
with tabs[2]:
    ...
    st.markdown('<div class="brand-line"></div>', unsafe_allow_html=True)
    st.info("Forecast Input relates only to incremental recurring revenues and costs generated by the new investment. Existing business volumes should not be included in this section. MOH should exclude depreciation costs; depreciation is entered separately in the dedicated depreciation columns.")
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.checkbox("Material: enter Value manually → calculate %", key="manual_material_value")
    with c2:
        st.checkbox("Direct Labour: enter Value manually → calculate %", key="manual_labour_value")
    with c3:
        st.checkbox("MOH: enter Value manually → calculate %", key="manual_moh_value")
    with c4:
        st.checkbox("SG&A: enter Value manually → calculate %", key="manual_sga_value")
    ...
    col_cfg = {
        ...
        "Depreciation of Investment": st.column_config.NumberColumn("Depreciation of Investment", format="%.0f", min_value=0.0, help="Yearly depreciation of the project investment. If entered, it overrides automatic depreciation from Project Setup for that year."),
        "Depreciation of Other Assets": st.column_config.NumberColumn("Depreciation of Other Assets", format="%.0f", min_value=0.0, help="Yearly depreciation/amortisation of other fixed assets to include in P&L."),
    }
    disabled_cols = ["Year"]
    disabled_cols.append("Material %" if st.session_state.manual_material_value else "Material Value")
    disabled_cols.append("Direct Labour %" if st.session_state.manual_labour_value else "Direct Labour Value")
    disabled_cols.append("MOH %" if st.session_state.manual_moh_value else "MOH Value")
    disabled_cols.append("SG&A %" if st.session_state.manual_sga_value else "SG&A Value")
    ...
    edited = _clean_forecast_df(edited)
